...
class SaasController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                ->addIndexColumn()
                ->addColumn('action', function ($row) {
                    return '<a href="' . route('saas.edit', $row->id) . '" class="badge bg-primary"><i class="fas fa-edit"></i></a> <a href="' . route('customer.view', $row->id) . '" class="badge bg-info"><i class="fas fa-eye"></i></a> <a onclick="return confirm(\'Are you sure?\')" href="' . route('saas.delete', $row->id) . '" class="badge bg-danger"><i class="fas fa-trash"></i></a>';
                })
                ->rawColumns(['action'])
                ->make(true);
        }
        ...
        return view('saas.list', compact('customer'));
    }

    public function edit_shop($id){
        $customer = Customer::find($id);
        // dd($customer->id);
        return view ('saas.edit', compact('customer'));
    }
    public function add_shop(Request $request){
        ...
        $package  = Package::all();
        $method   = Method ::all();
        return view ('saas.add', compact('customer', 'district', 'package', 'method'));
    }
    ...
    public function shop_store(Request $request){
        ...
        $shop->method_id = $request->method_id;
    }
