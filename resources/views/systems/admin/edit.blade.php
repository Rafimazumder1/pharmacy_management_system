            ...
            <div class="card-body">
                <form action="{{ route('admin.update', $admin->id) }}" method="post" class="row">
                    ...
                    <div class="form-group col-lg-4">
                        <label class="req">Shop</label>
                        <select class="form-select" name="shop_id">
                            <option selected value="">Select Shop</option>
                            @foreach ($shops as $shop)
                                <option value="{{ $shop->id }}" @if ($admin->shop_id == $shop->id) selected @endif>
                                    {{ $shop->name }}</option>
                            @endforeach
                        </select>
                        @error('shop_id')
                            <span class="text-danger">Shop is required!</span>
                        @enderror
                    </div>
                    ...
                    @if (Auth::user()->role_id == 1)
                        <div class="form-group mt-1 col-lg-4">
                            <label class="req">Update Password</label>
