...
class InvoiceController extends Controller
{
    ...
    public function allreports(Request $request)
    {
        ...
        foreach (array_reverse($dates) as $date) {
            $data['date'] = $date;
            $data['total_sele'] = Invoice::where('SHOP_ID', Auth::user()->shop_id)->where('N_DATE', $date)->count();
            $data['total_sale_price'] = Invoice::where('SHOP_ID', Auth::user()->shop_id)->where('N_DATE', $date)->sum('TOTAL_PRICE');
            $data['total_sale_amount'] = InvoicePay::where('SHOP_ID', Auth::user()->shop_id)->where('NEW_DATE', $date)->sum('AMOUNT');
            $data['total_purchase'] = Purchase::where('SHOP_ID', Auth::user()->shop_id)->where('NEW_DATE', $date)->count();
            $data['total_purchase_price'] = Purchase::where('SHOP_ID', Auth::user()->shop_id)->where('NEW_DATE', $date)->sum('TOTAL_PRICE');
            $data['total_purchase_amount'] = Purchase::where('SHOP_ID', Auth::user()->shop_id)->where('NEW_DATE', $date)->sum('AMOUNT');
